...
export default function Table({
  ...
  return (
    <table
      className={`${classes} w-full text-sm rounded border border-separate border-spacing-0 border-gray-l3 dark:border-blue-gray`}
    >
      <TableSection
        type="thead"
        ...
        {sorted
          .map((row) => (
            <Cells
              key={row.PK}
              type="td"
              cellClass={`p-3 border-t border-gray-l3 dark:border-blue-gray max-w-[256px] truncate ${
          ...
          .concat(
            hasMore ? (
              <td
                colSpan={9}
                key="load-more-btn"
                className="border-t border-gray-l3 dark:border-blue-gray rounded-b"
              >
                <LoadMoreBtn
