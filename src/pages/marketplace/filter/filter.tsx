...
export default function Filter({ classes = "" }: { classes?: string }) {
  ...
  return (
    ...
    >
      ...
      <DialogPanel
        className={`${classes} fixed-center z-50 isolate w-full max-w-[95vw] max-h-[95vh] sm:max-w-md overflow-y-auto scroller border border-gray-l3 bg-gray-l6 dark:bg-blue-d5 dark:text-white rounded-sm`}
      >
        <div className="bg-blue-l5 dark:bg-blue-d7 flex items-center p-4 border-b border-gray-l3">
          <p className="font-semibold  uppercase mr-auto">Filters</p>
          <button
            type="button"
        ...
        <div className="px-4 py-4">
          <label className="font-semibold text-xs  uppercase block mb-2">
            Countries
          </label>
          <Countries />
        </div>
