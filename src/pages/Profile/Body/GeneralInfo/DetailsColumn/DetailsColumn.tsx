...
export default function DetailsColumn({ className = "" }) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div
        className={`${className} flex flex-col gap-8 w-full lg:w-96 p-8 border border-prim rounded text-gray-d2 dark:bg-blue-d4 dark:text-white`}
      >
...
function Detail(props: PropsWithChildren<{ title: string }>) {
  return (
    <div className="flex flex-col justify-center items-start gap-2 w-full">
      ...
      <span className="font-body font-normal text-base text-gray-d1 dark:text-gray-l3">
        {props.children || "-"}
      </span>
    </div>
  );
