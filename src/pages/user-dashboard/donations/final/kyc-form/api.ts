import { toPrettyUtc } from "@better-giving/helpers";
...
import { SendMessageCommand, sqsc_ap } from ".server/aws/sqs";
import { bg_npo_id, q_url_email } from ".server/env";
...
export const action = async ({ request, params }: Route.ActionArgs) => {
  ...
  const donor: EmailSQS.Donation.Donor = {
  ...
  const tx: EmailSQS.Donation.Tx = {
    transactionID: don.transactionId,
    transactionDate: toPrettyUtc(don.transactionDate),
    ...
    isBg: don.endowmentId === bg_npo_id,
    isRecurring: don.isRecurring ?? false,
  };
  //but can send receipt to other email
  const data: EmailSQS.Donation.ReceiptData = {
    ...tx,
    donor: donor,
  ...
  const payload: EmailSQS.Donation.Payload = {
    template: "donation-receipt",
    // user can send receipt other than their login/donation email
    recipients: [fv.data.email],
    data: data,
  };

  const cmd = new SendMessageCommand({
    QueueUrl: q_url_email,
    MessageBody: JSON.stringify(payload),
  });
  const res = await sqsc_ap.send(cmd);
  console.info(res);
