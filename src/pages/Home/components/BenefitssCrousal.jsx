...
const BenefitssCrousal = ({ path, slides }) => {
  ...
  return (
    <div className="lg:flex justify-between w-full  items-center p-16  hidden relative">
      <div className="swip-prev slider-arrow relative  left-[12.5%] translate-x-[-50%]  z-40   borderLine rounded-full shadow-md p-4"  style={{ opacity: showPrevButton ? 1 : 0 }}>
...
// }}
      ...
      >
        {slides.map((ele,index) => {
          ...
          // console.log("Anurag , ",index);
          return (
            // className={index === activeIndex  ? 'red-slide' : 'yellow'}
            <SwiperSlide key={index}  style={{ backgroundColor: colors[index] }} >
              <img src={ele.img_src}  className="w-60 h-60 mb-3 object-cover object-center" alt="logo" />
              <p className="text-2xl text-[#0D283A] w-full DM_Sans  font-semibold">{ele.title}</p>
              <p className="text-base mt-2 text-ellipsis md:text-center DM_Sans font-normal w-full text-[#1D3C51]">{ele.description}</p>
            </SwiperSlide>
          );
        })}
      </Swiper>
      ...
      {/*  style={{ display: showNextButton ? "block" : "none" }} */}
       ...
       <div className="swip-next slider-arrow relative  right-[8%] z-30 translate-x-[-50%]   borderLine rounded-full p-4 shadow-md " style={{ opacity: showNextButton ? 1 : 0 }} >
      ...
      <div className='absolute bottom-[10px] right-[12%] flex flex-col items-center -rotate-12'>
                ...
                <p className='Gochi_Hand max-w-[70%] text-center text-lg font-medium text-[#111] '>Scroll to know more</p>
            </div>
    </div>
  );
};
