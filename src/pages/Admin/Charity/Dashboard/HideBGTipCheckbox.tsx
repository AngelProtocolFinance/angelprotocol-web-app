...
export default function HideBGTipCheckbox() {
  ...
  const queryState = useEndowment(id, ["hide_bg_tip"]);
  return (
    <QueryLoader
      queryState={queryState}
      messages={{ loading: <LoaderRing thickness={10} classes="w-6" /> }}
    >
      {({ hide_bg_tip }) => <Content value={hide_bg_tip} />}
    </QueryLoader>
  );
}
...
function Content({ value = false }) {
  ...
  const methods = useForm({
    resolver: yupResolver(object({ value: bool().required() })),
    defaultValues: { value },
    mode: "onChange",
  });
  ...
  const submit = async (fv: FV) => {
    try {
      await toggleBgTipScreen({ id, hide_bg_tip: fv.value }).unwrap();
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <CheckField<FV> name="value">Opt out of tip-based donations</CheckField>
      <button
        className="justify-self-end btn-orange w-40"
        disabled={dirtyFields.value == null}
      >
        Save
      </button>
    </Form>
