...
const TestimonialCard = (props) => {
  return (
    <div className={`relative md:w-full  md:min-h-[450px] lg:w-full lg:min-h-[450px] w-full min-h-[430px] p-[12px] pt-[62px] borderLine rounded-3xl flex flex-col justify-between bg-white   font-body  overflow ${props.className}`}>
      {props.role==="ADMIN" && <CRUDButtons handleDelete={props.handleDelete} id={props.id} />}
        ...
        <img src={props.reviewer_logo} alt="logo" className='w-full h-full object-contain '/> 
        </div>
        ...
        <span className='h-[84px]'>
            ...
            <p className='lg:text-lg md:text-lg text-[16px] font-medium opacity-75'>{props.reviewer_profession}</p>
        </span>
    </div>
  )
}
