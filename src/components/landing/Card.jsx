  ...
  return (
    <div className="flex flex-col gap-8 w-[90%] max-h-[520px] md:w-full md:max-h-[520px] lg:w-full lg:max-h-[520px] bg-white pb-4 rounded-[20px] z-10 overflow-hidden shadow_1 font-body">
      <span className="relative">
        <img src={props.img_src} alt="card img" className="w-full h-[240px] " />
        <img
          src={props.logo}
          alt="logo"
      ...
      <span className="px-8 justify-center flex items-center gap-8">
        <button className="rounded-[40px] py-2 px-7 border-2 border-solid border-[#2D89C8] text-[#2D89C8] font-semibold font-heading">
          {" "}
          <Link to={`https://app.better.giving/marketplace/${props.id}`}>
            Donate
          </Link>
        </button>
      </span>
