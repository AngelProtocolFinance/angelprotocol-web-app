...
coverflowEffect={{
  ...
  modifier: 3.8,
        ...
        {slides.map((ele,index) => {
          return (
            <SwiperSlide key={index}  style={{ backgroundColor: colors[index] }} >
              ...
              <p className="text-2xl text-[#0D283A] w-full font-body  font-semibold">{ele.title}</p>
              <p className="text-base mt-2 text-ellipsis md:text-center font-body font-normal w-full text-[#1D3C51]">{ele.description}</p>
            </SwiperSlide>
          );
        })}
      </Swiper>
      ...
      <div className='absolute bottom-[10px] right-[12%] flex flex-col items-center -rotate-12'>
                ...
                <p className='font-gochi max-w-[70%] text-center text-lg font-medium text-[#111] '>Scroll to know more</p>
            </div>
    </div>
  );
};
