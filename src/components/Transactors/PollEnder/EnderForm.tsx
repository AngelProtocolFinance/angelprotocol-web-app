import useEndPoll from "./useEndPoll";

export default function EnderForm(props: { poll_id: number }) {
  const { endPoll } = useEndPoll(props.poll_id);
  return (
    <div className="bg-white grid justify-items-center p-4 rounded-md w-full">
      <p className="text-gray-d2">are you sure you want to end this poll?</p>
      <button
        type="button"
        className="rounded-md bg-orange text-white hover:text-gray-d2 font-heading px-4 py-1 text-xs uppercase font-bold mt-4"
        onClick={endPoll}
      >
        confirm
      </button>
    </div>
  );
}
