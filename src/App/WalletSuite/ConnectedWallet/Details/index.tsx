...
export default function Details(props: WalletState) {
  return (
    <Popover.Panel className="fixed sm:absolute inset-0 sm:inset-auto sm:origin-top-right sm:mt-2 sm:right-0 flex flex-col sm:w-80 h-full sm:h-fit bg-white dark:bg-blue-d6 sm:rounded-lg border border-gray-l2 dark:border-bluegray shadow-[0_0_16px_rgba(15,46,67,0.25)] text-gray-d2 dark:text-white">
      {({ close }) => (
        <>
...
function DisconnectBtn() {
  ...
  return (
    <button
      onClick={disconnect}
      className="btn h-12 bg-orange-l5 dark:bg-blue-d5 hover:bg-orange-l3 dark:bg-blue-d7 uppercase font-body font-bold text-base sm:rounded-b-lg"
    >
      disconnect
    </button>
  );
