      ...
      <div className="grid grid-cols-[auto_1fr] py-3 gap-x-2">
        {linkGroups.map((group) => (
          <React.Fragment key={`link_group-${group.title}`}>
            {group.title && (
              <h6 className="col-span-2 pt-5 px-5 pb-1 font-semibold text-xs uppercase text-gray tracking-wide">
                {group.title}
              </h6>
            )}
            {group.links.map((link) => (
              <NavLink
                ...
                className={({ isActive, isPending }) =>
                  `grid grid-cols-subgrid col-span-2 items-center py-3 px-5 font-semibold text-sm hover:text-blue-d1 transition ease-in-out duration-300 aria-disabled:text-gray aria-disabled:pointer-events-none 
